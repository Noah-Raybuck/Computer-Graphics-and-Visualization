...
namespace
{
    const char* const WINDOW_TITLE = "Assignment 6"; // Macro for window title
    ...
    glm::vec3 gLightPosition(2.0f, 0.0f, 0.0f);
    glm::vec3 gLightScale(0.0f);

    // Light 2 color
    glm::vec3 gLight2Color(0.5f, 1.0f, 0.5f);

    // Light 2 position and scale
    glm::vec3 gLight2Position(-2.0f, 0.0f, 3.0f);
    glm::vec3 gLight2Scale(0.0f);
...
void main()
{
    ...
    //Calculate Ambient lighting 2*/
    float ambientStrength2 = 1.0f; // Set ambient or global lighting strength
    vec3 ambient2 = ambientStrength2 * light2Color; // Generate ambient light color

    //Calculate Diffuse lighting 2*/
    ...
    float specularComponent2 = pow(max(dot(viewDir, reflectDir), 0.0), highlightSize2);
    ...
    vec3 phong = (ambient + diffuse + specular + ambient2 + diffuse2 + specular2) * textureColor.xyz;
...
int main(int argc, char* argv[])
{
    ...
    // Load texture
    const char* texFilename = "brick_texture.jpg";
